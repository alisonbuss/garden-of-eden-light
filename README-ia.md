...
Um orquestrador **leve e objetivo** de Shell Scripts para provisionar ambientes em Linux (focado em distros baseadas em Ubuntu/Debian), inspirado no projeto original **Garden of Eden**, mas propositalmente simplificado.
...
3. **Stack completa de ferramentas**  
   - **Base de sistema e utilitários**: ferramentas comuns, Flatpak, Chrome, Hyper, Git etc.  
   ...
   - **Trading/cripto**: ProtonVPN, Exodus Wallet, Ledger Live, TradingView Desktop, yt-dlp, Bottles.
...
- Útil para:
  - Máquina pessoal onde você quer ter tudo (dev + trading + infra).  
...
> **Perfis padrão** indicam em quais arquivos cada script aparece (`all`, `dev`, `trading`).  
...
- **Projeto original**: Garden of Eden (orquestrador completo, com módulos, actions e parâmetros JSON).  
- **Documentação oficial das ferramentas**:  
  Cada script traz no cabeçalho (`@fonts`) links para a documentação oficial da ferramenta correspondente, como:
  - Docker, Docker Compose  
  - Terraform, Packer, Ansible, AWS CLI  
  - Flatpak, Flathub  
  - VS Code, Spring Tools, DBeaver, Postman, Insomnia, Hoppscotch  
  - ProtonVPN, Exodus, Ledger, TradingView etc.
